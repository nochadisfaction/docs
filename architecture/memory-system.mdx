...
description:
  'How Pixelated Empathy implements long-term memory using Mem0 + Google Gemini
...
The `GeminiMem0Manager` is the core orchestrator of the memory system. It
integrates Google Gemini (for PII filtering and response generation) with Mem0
...
<Tip>
  The `TherapeuticMemoryConfig` uses **Mem0 cookbook best practices** for
...
The memory system uses **Mem0's custom instruction feature** to guide what gets
extracted and stored. These instructions are applied at the project level and
influence Mem0's memory extraction behavior.
...
<Info>
  ...
  the Mem0 Platform Client fails to initialize, the manager falls back to local
  Mem0. If that also fails, `NullMemoryManager` is used for the lifetime of the
